...
def get_clean_seqs():
    ...
    file_dir=os.path.join('D:/Phdworks/data/vocation/annotation','test')
    ...
    for fidx in range(len(files)):
        ...
        data=np.load(os.path.join(file_dir,file),allow_pickle=True)
        ...
        for seq_id in range(seq_cnt):
            ...
            if len(sequences_all[seq_id])>=50:
                ...
                if res:
                    ...
                    cnt=int(len(sequences_all[seq_id])/50)
                    print(cnt)

                    for i in range(cnt):

                        seq_sec=sequences_all[seq_id][i*50:(i+1)*50]

                        save_name = os.path.join('D:/Phdworks/data/vocation/annotation/', 'seq20','test', 'vid_' + vid + '_seq_' + str(seq_save_cnt) + '.npy')
